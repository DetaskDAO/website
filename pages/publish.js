...
import { ConfigTask } from "../src/controller/index";
// request
import { createTask, getSillTreeMap, updateTask } from "@/request/_api/task";
import { getHash, searchTaskDetail } from "@/request/_api/public";
// utils
...
export default function Publish() {
  ...
  const writeSuccess = (hash, modify) => {
    if (modify) {
      updateTask({ hash: hash }).then((res) => {
        if (res.code === 0) {
          setShowSpin(true);
        }
      });
    } else {
      createTask({ hash: hash }).then((res) => {
        if (res.code === 0) {
          setShowSpin(true);
        } else {
          message.error(res.msg);
  ...
  useEffect(() => {
    if (location.search) {
      ...
      searchTaskDetail({ issuer: address, task_id: id }).then((res) => {
        if (res.code === 0 && res.data.list && res.data.list.length > 0) {
          modifyInfo = res.data.list[0];
          modifyInfo.currency =
            modifyInfo.currency === "USD" ? "USDC" : modifyInfo.currency;
          modifyInfo.budget = taskCurrency(
            modifyInfo.currency,
            modifyInfo.budget
          );
                ...
                <div className="item">
                  <Form.Item>
                    <Button
                      ...
                      htmlType="submit"
                      disabled={isPost}
                      className={`panel- ${!isPost ? "panel-btn" : ""}`}
                      loading={createLoading || modifyLoading}
                    >
                      {modifyInfo ? t("btnmodify") : t("btn")}
                    </Button>
                  </Form.Item>
                </div>
              </Form>
